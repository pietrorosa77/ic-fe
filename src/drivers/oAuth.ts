  ...
  function oAuthDriver(
    ...
    oauthEndpoint$.addListener({
      next: item => {
        ...
        const url = getProviderAuthURL({ ...item, returnUrl, csrfToken })
        localStorage.setItem(`OAUTHLOGIN`, JSON.stringify({ csrfToken, provider: item.provider }));
...
function getProviderAuthURL(item:{
  ...
  const url = R.compose(R.concat(`${item.authorizeUrl}?`),R.join("&"),R.map(R.join("=")),R.toPairs, mapAuthItemToAuth0Querystring)(item);
  return url;
}
...
function getAuthReturn() {
  ...
  const ret = R.compose(R.fromPairs , R.map(R.split("=")) as any, R.split("&"), R.tail)(window.location.search) as any
